...
def check(ctx):
    ...
    if ('good bot' in ctx) or ('bad bot' in ctx):
        ...
        if ('bad bot') in ctx:
            ...
            msg = ':rage: You :rage: brought :rage: my :rage: thanks :rage: to :rage: ' + str(
                data['dayvalues']['goodness']) + 'th :rage: hope :rage: you :rage: feel :rage: good :rage:'
        if ('good bot') in ctx:
            data['dayvalues']['goodness'] = data['dayvalues']['goodness'] + 1
...
async def default_dance(message, client):
    for x in range(0, 2):
        if x == 0:
